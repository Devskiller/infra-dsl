...
class ApiDocGenerator {
	...
	final private Class rootClass = Infrastructure
	...
	private List getMethodsForClass(Class clazz) {
		List<String> methodNames = clazz.methods.findAll {
			!it.name.startsWith('get') &&
					...
					!it.name.contains('$') &&
					!isAnnotatedWIthNoDoc(it) &&
					!(it.name in ['invokeMethod', 'executeWithXmlActions', 'methodMissing', 'render', 'renderElement'])
		}*.name.unique().sort()
		methodNames.collect { processMethodName it, clazz }
	}

	private boolean isAnnotatedWIthNoDoc(Method it) {
		NoDoc noDoc = it.getAnnotation(NoDoc)
		if (noDoc == null && it.getDeclaringClass() != Object) {
			try {
				Method method = it.getDeclaringClass().getSuperclass().getMethod(it.name, it.parameterTypes)
				return isAnnotatedWIthNoDoc(method)
			} catch (NoSuchMethodException e) {
				return false
			}
		}
		return noDoc && !noDoc.embeddedOnly()
	}
